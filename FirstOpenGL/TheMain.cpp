...
//static const int MAXNUMBEROFGAMEOBJECTS = 25;
...
//int x[10];			// entirely on the 'stack' (1Mybte)
//int* x = new int[10];	// Actual array is on the heap
...
int main(void)
{
	...
	//// Fills array with zeros (NULLs)
	//for ( int index = 0; index != MAXNUMBEROFGAMEOBJECTS; index++ )
	//{	// cGameObject* 
	//	::g_GameObjects[index] = 0;
	//	// Could set this to NULL, nullptr 
	//}

	//// "Add" a bunny  (cGameObject*, not cGameObject )
	//::g_GameObjects[0] = new cGameObject();
	//::g_GameObjects[0]->position.x = 0.5f;
	//::g_GameObjects[0]->orientation.z = glm::degrees(0.0f);	// Degrees
	//::g_GameObjects[0]->orientation2.z = glm::degrees(0.0f);	// Degrees
	//::g_GameObjects[0]->scale = 2.0f;
	//::g_GameObjects[0]->diffuseColour = glm::vec4(1.0f, 0.0f, 0.0f, 1.0f);
	//::g_GameObjects[0]->meshName = "bunny";
	...
	for (int index = 3; index < 20 ; index++)
	{
		...
		//pTempGO->scale = getRandInRange<float>( 7.0f, 15.0f );
